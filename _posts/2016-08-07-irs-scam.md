...
* One which returns the entirety of the array
* Another which retrieves an element from the array based on a passed index
* A third which unshuffles the array, so that it can be used by the rest of the scripts 

The first function is straightforward. It contains the array, and returns it.
...
To retrieve these elements, we use the very same get_word_from_array(*index*) function that we used for unshuffling the array. This process of calling the function must be repeated for almost every line of obfuscated code to derive its true purpose -- a time perfect for medidation or hobbyist dissociation.
